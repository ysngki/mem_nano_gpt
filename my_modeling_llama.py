...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if input_parameter is not None and peft == "lora":
            this_input_parameter = input_parameter.to(hidden_states.device).to(hidden_states.dtype)

            r = this_input_parameter.shape[1] // 2
            down_matrix = this_input_parameter[:, :r, :] # (batch size, r, dim)
            up_matrix = this_input_parameter[:, r:, :] # (batch size, r, dim)

            intermediate_weight = self.mlp.act_fn(torch.matmul(hidden_states, down_matrix.transpose(1, 2))) # (batch size, seq len, r)
            new_hidden_states = torch.matmul(intermediate_weight, up_matrix) # (batch size, seq len, dim)

            hidden_states = self.mlp(hidden_states)
            hidden_states = residual + hidden_states + new_hidden_states
        else:
            hidden_states = self.mlp(hidden_states)
            hidden_states = residual + hidden_states
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    input_parameter=input_parameter[idx] if input_parameter is not None else None,
                    peft=peft,
                )
