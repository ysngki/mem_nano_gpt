...
_CONFIG_FOR_DOC = "MemoryRobertaConfig"
...
class RobertaPreTrainedModel(PreTrainedModel):
    ...
    config_class = MemoryRobertaConfig
...
@add_start_docstrings(
    "Roberta Model with memory.",
    ROBERTA_START_DOCSTRING,
)
class MemoryRobertaModel(RobertaPreTrainedModel):
    """
    ...
    def forward(
        ...
        if self.num_memory > 0:
            prefix_attention_mask = torch.ones(((batch_size, self.num_memory)), device=device)
        ...
        # perfix memory
        actual_memory_len = 0
        if not(input_memory is None):
            embedding_output = torch.cat((input_memory, embedding_output), dim=1)
            actual_memory_len = input_memory.shape[1]
        elif self.num_memory > 0:
            prefix_memory = self.initial_memory.repeat(batch_size, 1, 1)
            embedding_output = torch.cat((prefix_memory, embedding_output), dim=1)
            actual_memory_len = self.num_memory
        ...
        if actual_memory_len > 0:
            new_memory = sequence_output[:, :actual_memory_len, :] # (batch size, memory len, dim)
        else:
            new_memory = None

        if not return_dict:
            return (sequence_output, pooled_output, new_memory) + encoder_outputs[1:]

        return MemoryBaseModelOutputWithPoolingAndCrossAttentions(
            last_hidden_state=sequence_output,
            pooler_output=pooled_output,
            memory_output=new_memory,
            past_key_values=encoder_outputs.past_key_values,
            hidden_states=encoder_outputs.hidden_states,
            attentions=encoder_outputs.attentions,
            cross_attentions=encoder_outputs.cross_attentions,
...
@dataclass
class MemoryBaseModelOutputWithPoolingAndCrossAttentions(ModelOutput):
    """
    ...
    memory_output: torch.FloatTensor = None
