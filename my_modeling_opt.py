    ...
    def forward(
        ...
        if input_parameter is not None and peft == "prompt":
            ...
            attention_mask = torch.cat([
                ...
                torch.ones((batch_size, memory_len), dtype=attention_mask.dtype, device=device), 
                attention_mask[:, past_key_values_length:]
                ], dim=-1)

        causal_attention_mask = self._prepare_decoder_attention_mask(
            attention_mask, (batch_size, memory_len + seq_length), inputs_embeds, past_key_values_length
        )
...
class OPTForCausalLM(OPTPreTrainedModel):
    ...
    def forward(
        ...
        if output_embeds:
            ...
            pos_embeds = self.model.decoder.embed_positions(attention_mask)
